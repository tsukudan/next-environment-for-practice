...
- [AWS ECS デプロイパイプライン 構築手順書](#aws-ecs-デプロイパイプライン-構築手順書)
  ...
  - [6. リソース削除手順](#6-リソース削除手順)
    - [依存関係の全体図](#依存関係の全体図)
    - [削除手順（順番厳守）](#削除手順順番厳守)
      - [1. ECSサービスのタスクを停止する](#1-ecsサービスのタスクを停止する)
      - [2. ECSサービスを削除する](#2-ecsサービスを削除する)
      - [3. ECSクラスターを削除する](#3-ecsクラスターを削除する)
      - [4. ECSタスク定義を登録解除する](#4-ecsタスク定義を登録解除する)
      - [5. ALBを削除する](#5-albを削除する)
      - [6. ターゲットグループを削除する](#6-ターゲットグループを削除する)
      - [7. VPCエンドポイントを削除する（4つ）](#7-vpcエンドポイントを削除する4つ)
      - [8. VPCを削除する](#8-vpcを削除する)
      - [9. ECRリポジトリを削除する](#9-ecrリポジトリを削除する)
      - [10. CloudWatch Logsグループを削除する](#10-cloudwatch-logsグループを削除する)
      - [11. IAMロールを削除する](#11-iamロールを削除する)
      - [12. IAM OIDCプロバイダーを削除する](#12-iam-oidcプロバイダーを削除する)
    - [削除後のコスト確認](#削除後のコスト確認)
...
コスト削減や環境の作り直しのために全リソースを削除する手順です。  
AWSリソースには依存関係があるため、**依存されている側（子）から先に削除**する必要があります。
...
VPC
  ├─ サブネット（ECS・ALBが使用）
  ├─ セキュリティグループ（ECS・ALB・VPCエンドポイントが使用）
  └─ VPCエンドポイント（ECSがECR・Logsに接続するために使用）
```

**削除の方向**: 図の末端（最も多くのものに依存している側）から削除する。
...
**理由**: サービスにタスクが稼働中だと削除できない。先にタスク数を0にしてタスクを停止させる。

ECS → `next-practice-cluster` → `next-practice-frontend-svc` → 「サービスを更新」
...
**理由**: ECSクラスターはサービスが存在すると削除できない。

ECS → `next-practice-cluster` → サービス → `next-practice-frontend-svc` → 「削除」

#### 3. ECSクラスターを削除する

**理由**: ALBターゲットグループはECSサービスが登録されている間は削除できない。サービスを削除した後であればクラスターを削除できる。

ECS → `next-practice-cluster` → 「削除」

#### 4. ECSタスク定義を登録解除する

**理由**: タスク定義はサービスから参照されなくなった後に登録解除できる。削除ではなく「登録解除」になる（AWSの仕様）。

ECS → タスク定義 → `next-practice-frontend` → 全リビジョンを選択 → 「登録解除」

#### 5. ALBを削除する

**理由**: ALBはリスナーを内包しているため、ALBを削除するとリスナーも同時に削除される。ターゲットグループより先に削除する必要はないが、ALBが残っているとターゲットグループの削除が拒否されることがある。

EC2 → ロードバランサー → `next-practice-alb` → 「削除」

#### 6. ターゲットグループを削除する

**理由**: ALBのリスナーから参照されている間は削除できない。ALB削除後に削除する。

EC2 → ターゲットグループ → `next-practice-tg` → 「削除」

#### 7. VPCエンドポイントを削除する（4つ）

**理由**: Interface型エンドポイントはセキュリティグループを使用しており、エンドポイントを削除する前にセキュリティグループは削除できない。

VPC → エンドポイント → 以下4つを選択 → 「削除」

- `next-practice-vpce-ecr-api`
- `next-practice-vpce-ecr-dkr`
- `next-practice-vpce-logs`
- `next-practice-vpce-s3`

#### 8. VPCを削除する

**理由**: VPCはサブネット・セキュリティグループ・インターネットゲートウェイ・ルートテーブルを内包している。コンソールの「VPCを削除」では関連リソースを一括削除できる。ECS・ALB・VPCエンドポイントをすべて削除した後でなければVPCは削除できない。

VPC → `next-practice-vpc` → 「VPCを削除」

> 「関連するすべてのリソースを削除する」にチェックを入れると、サブネット・セキュリティグループ・IGW・ルートテーブルが一括削除される。

#### 9. ECRリポジトリを削除する

**理由**: イメージが残っていても削除できるが、「強制削除」オプションが必要。

ECR → `next-practice/frontend` → 「削除」（「強制削除」にチェック）

#### 10. CloudWatch Logsグループを削除する

**理由**: ECS削除後も自動では削除されず、ログが残り続けてストレージ課金が発生する。

CloudWatch → ロググループ → `/ecs/next-practice/frontend` → 「削除」

#### 11. IAMロールを削除する

**理由**: IAMロールはECSタスク定義・ECSサービス・OIDCプロバイダーから参照されているが、それらを削除した後であれば削除できる。

IAM → ロール → 以下を削除:
- `ecsTaskExecutionRole`
- `GitHubActions-next-practice-deploy`

> `AWSServiceRoleForECS` は削除不要。次回ECSを使うときにも必要なサービスリンクロールのため残しておく。

#### 12. IAM OIDCプロバイダーを削除する

**理由**: GitHub ActionsのOIDC認証用に作成したプロバイダー。IAMロールを削除した後に削除できる。

IAM → IDプロバイダー → `token.actions.githubusercontent.com` → 「削除」

---

### 削除後のコスト確認

削除完了後、AWS Cost Explorerで翌日以降の課金が止まっていることを確認することを推奨します。  
特にInterface型VPCエンドポイント（3つで約$30/月）とALB（約$17/月）の課金が止まっているかを確認してください。
